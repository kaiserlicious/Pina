...
THIS::THIS(Document* d, TiXmlHandle h):Element(d,h){
  ...
  /* data */
  if(handle.ToElement()){
    version = handle.ToElement()->GetText();
  }
...
void THIS::order(){ children.sort(Ordering<Types>()); } const std::string THIS::Name = "revision";
...
TiXmlElement* THIS::toTiXmlElement(){
  ...
  TiXmlText* text = new TiXmlText(Utils::toString(version));
